...
$manifest=Get-Content -LiteralPath (Join-Path $PSScriptRoot 'upgrade-0.7.2.json') -Raw | ConvertFrom-Json
...
$backupDir=Join-Path $GamePath 'WGame\ModBackups\WolverineAnatomyTool-v0.7.2-r2'
...
if($GamePath -eq 'C:\Games\X-Men Origins Wolverine' -and (Get-Process -Name Wolverine -ErrorAction SilentlyContinue)){throw 'Close Wolverine before upgrading or rolling back.'}
...
if($Rollback){
    if(-not(Test-Path -LiteralPath $statePath)){throw 'No 0.7.2-r2 upgrade backup was found.'}
...
$current=Hash $target
if($current -eq $manifest.runtimeSHA256){Write-Output 'v0.7.2-r2 is already installed.';return}
if($manifest.acceptedRuntimeSHA256 -notcontains $current){throw 'This upgrade requires the released 0.7, original 0.7.1, 0.7.1 junction R2, or 0.7.2 runtime. No files were changed.'}
...
$settingsBackup=Join-Path $backupDir 'WolverineLive-before-0.7.2-r2.ini'
...
$state=@{Version='0.7.2-r2';GamePath=$GamePath;Status='Pending';OriginalReadOnly=$readOnly;PreviousHash=$current;InstalledHash=$manifest.runtimeSHA256}
...
Write-Output 'v0.7.2-r2 installed. Launch normally; F6 shows the version. Package and saved settings were preserved.'
