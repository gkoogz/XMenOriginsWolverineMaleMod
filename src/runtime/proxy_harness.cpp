...
static void ReadGraft(IDirect3DVertexBuffer9* vb,std::vector<P3>& out){void* raw=nullptr;out.resize(2304);vb->Lock(47050u*32u,2304u*32u,&raw,D3DLOCK_READONLY);for(unsigned i=0;i<2304;i++)memcpy(&out[i],(unsigned char*)raw+i*32u,12);vb->Unlock();}
static void LobeCenters(const std::vector<P3>& points,P3 centers[2]){unsigned counts[2]{};centers[0]=centers[1]=P3{};for(unsigned i=0;i<2304;i++){if(phys_scrotum_weight[i]<.82f)continue;int side=points[i].y<0?0:1;centers[side].x+=points[i].x;centers[side].y+=points[i].y;centers[side].z+=points[i].z;counts[side]++;}for(int side=0;side<2;side++){centers[side].x/=counts[side];centers[side].y/=counts[side];centers[side].z/=counts[side];}}
static void LobeIntegrity(const std::vector<P3>& before,const std::vector<P3>& after,float& rms,float& maximum){float maxY=0;for(unsigned i=0;i<2304;i++)if(phys_scrotum_weight[i]>.82f)maxY=max(maxY,fabsf(before[i].y));float coreCutoff=maxY*.40f;double error2=0;unsigned count=0;maximum=0;for(unsigned i=0;i<2304;i+=3){if(phys_scrotum_weight[i]<.82f||fabsf(before[i].y)<coreCutoff)continue;for(unsigned j=i+3;j<2304;j+=7){if(phys_scrotum_weight[j]<.82f||fabsf(before[j].y)<coreCutoff||(before[i].y<0)!=(before[j].y<0))continue;float d0=Distance(before[i],before[j]);if(d0<.35f)continue;float relative=fabsf(Distance(after[i],after[j])-d0)/d0;maximum=max(maximum,relative);error2+=relative*relative;count++;}}rms=count?sqrtf((float)(error2/count)):999.f;}
