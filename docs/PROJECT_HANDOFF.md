...
User confirmed the release should contain the currently installed build, including idle chatter, while the unresolved underside ridge remains documented. Branch `beta-1.0` starts from v0.9 and incorporates the later v0.9 publication record. Manifest v1.0.0-beta.1 names installed runtime SHA256 19717D7B... and records 22 WAV hashes. `payload/WolverineIdle` contains the three originals and 19 supplied clips. Installer v1.0.0-beta.1 backs up and verifies each clip, keeps saved settings, and uses a separate rollback directory from v0.9. Clean and upgrade fixtures passed reconstruction, payload and audio hashes, prior audio restoration, and exact rollback. ZIP `release/WolverineAnatomyTool-1.0-beta.1.zip` is 71,090,432 bytes; SHA256 3ef3dfafb7716c2cc34ea588d3bccb822c536effd8b466deae747149883f9315, verified against all manifest entries.

Source commit 8e7efd1 and merge commit a79357c were pushed to `origin/beta-1.0`. Annotated tag `v1.0.0-beta.1` points to a79357c. GitHub release `https://github.com/gkoogz/XMenOriginsWolverineMaleMod/releases/tag/untagged-d16cf58cba4259d16a7d` is a **draft prerelease** targeting a79357c, with archive and SHA256SUMS assets. GitHub reported archive digest SHA256 3ef3dfaf..., matching the local file. The draft remains unpublished.
