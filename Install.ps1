...
$backupRoot=Join-Path $GamePath 'WGame\ModBackups\WolverineAnatomyTool-v1.3.0'
$statePath=Join-Path $backupRoot 'state.json'
if($Mode -eq 'Uninstall'){
 $state=Get-Content -LiteralPath $statePath -Raw | ConvertFrom-Json
 if($state.GamePath -ne $GamePath -or $state.Version -ne '1.3.0'){throw 'Backup identity mismatch.'}
 if($state.Status -eq 'Uninstalled'){Write-Output '1.3 is already restored.';return}
...
if(Test-Path -LiteralPath $statePath){throw 'A 1.3 backup already exists. Restore it before another installation; retained backups are never overwritten.'}
...
try{
 ...
 $state=@{Version='1.3.0';GamePath=$GamePath;Status='Pending';Files=$records}
 ...
 Write-Output '1.3 installed and verified. F6 opens the controls. Saved settings preserved.'
}finally{
