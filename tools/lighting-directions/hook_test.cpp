...
#include "../../src/runtime/lighting_direction_fix.h"
...
static void checkedDraw(IDirect3DDevice9*d,IDirect3DVertexShader9*vs,IDirect3DPixelShader9*ps,bool expect){
 ...
 UINT before=lightingDirectionsDraws;require(SUCCEEDED(DrawWithLightingDirections(d,D3DPT_TRIANGLELIST,0,0,10554,0,21086)),"draw");
 require(lightingDirectionsDraws-before==(expect?1u:0u),"expected override count");
...
static std::string folder="C:/Games/X-Men Origins Wolverine/Binaries/R14Capture_20260920_023631/";
...
int main(int argc,char**argv){
 std::string mode=argc>1?argv[1]:"original";if(argc>2)folder=argv[2];
 ...
 auto bytes=read(folder+"draw_000_stream0.bin");if(mode=="sign"||mode=="sign255"||mode=="constant_tangent")for(unsigned i=47050;i<49438;i++){
 ...
 bytes=read(folder+"draw_000_indices.bin");IDirect3DIndexBuffer9* ib=nullptr;d->CreateIndexBuffer((UINT)bytes.size(),0,D3DFMT_INDEX16,D3DPOOL_MANAGED,&ib,nullptr);ib->Lock(0,0,&raw,0);memcpy(raw,bytes.data(),bytes.size());ib->Unlock();d->SetIndices(ib);
 bytes=read(folder+"draw_000_decl.bin");IDirect3DVertexDeclaration9* decl=nullptr;d->CreateVertexDeclaration((D3DVERTEXELEMENT9*)bytes.data(),&decl);d->SetVertexDeclaration(decl);
 auto v=read(folder+"draw_000_vs.txt"),p=read(folder+"draw_000_ps.txt");std::string vs(v.data()),ps(p.data());
 if(mode=="texcoord"){
 ...
 if(mode=="fresnel_world"){
  replace(vs,"    dcl_position o7", "    dcl_texcoord1 o8.xyz\n    dcl_texcoord2 o9.xyz\n    dcl_texcoord3 o10.xyz\n    dcl_position o7");
  replace(vs,"    mad r0, v1.xyzx", "    mul r9.xyz, r1.x, c230\n    mad r9.xyz, r1.y, c231, r9\n    mad o8.xyz, r1.z, c232, r9\n    mul r9.xyz, r4.x, c230\n    mad r9.xyz, r4.y, c231, r9\n    mad o9.xyz, r4.z, c232, r9\n    mul r9.xyz, r8.x, c230\n    mad r9.xyz, r8.y, c231, r9\n    mad o10.xyz, r8.z, c232, r9\n    mad r0, v1.xyzx");
  replace(ps,"    dcl vFace", "    dcl_texcoord1 v6.xyz\n    dcl_texcoord2 v7.xyz\n    dcl_texcoord3 v8.xyz\n    dcl vFace");

  replace(vs,"    dp3 o6.x, r1, c236", "");replace(vs,"    dp3 o6.y, r4, c236", "");replace(vs,"    dp3 o6.z, r8, c236", "");
  replace(vs,"    mul r3.xyz, r2.y, c235", "    mov o6.xyz, r2\n    mul r3.xyz, r2.y, c235");
  // Only the first r2 at this point is camera minus world position; the
  // same substring appears later for LightDirection and must not overwrite it.
  size_t first=vs.find("    mov o6.xyz, r2");size_t second=vs.find("    mov o6.xyz, r2",first+1);
  if(second!=vs.npos)vs.erase(second,std::string("    mov o6.xyz, r2\n").size());
  replace(ps,"    dcl vFace", "    dcl_texcoord7 v9.xyz\n    dcl vFace");
  replace(ps,"    dp3_sat r2.x, r2, r4", "    mul r14.xyz, v6, r2.x\n    mad r14.xyz, v7, r2.y, r14\n    mad r14.xyz, v8, r2.z, r14\n    nrm r15.xyz, r14\n    nrm r16.xyz, v9\n    dp3_sat r2.x, r15, r16");
 }
 if(mode=="view_world"||mode=="view_skin"){
  replace(vs,"    dcl_position o7", "    dcl_texcoord1 o8.xyz\n    dcl_texcoord2 o9.xyz\n    dcl_texcoord3 o10.xyz\n    dcl_position o7");
  replace(vs,"    mad r0, v1.xyzx", "    mul r9.xyz, r1.x, c230\n    mad r9.xyz, r1.y, c231, r9\n    mad o8.xyz, r1.z, c232, r9\n    mul r9.xyz, r4.x, c230\n    mad r9.xyz, r4.y, c231, r9\n    mad o9.xyz, r4.z, c232, r9\n    mul r9.xyz, r8.x, c230\n    mad r9.xyz, r8.y, c231, r9\n    mad o10.xyz, r8.z, c232, r9\n    mad r0, v1.xyzx");
  replace(ps,"    dcl vFace", "    dcl_texcoord1 v6.xyz\n    dcl_texcoord2 v7.xyz\n    dcl_texcoord3 v8.xyz\n    dcl vFace");
  ...
  replace(ps,"    nrm r4.xyz, v5", "    nrm r14.xyz, v8\n    dp3 r15.x, v6, r14\n    mad r15.xyz, r14, -r15.x, v6\n    nrm r16.xyz, r15\n    mul r15.xyz, r14.yzxw, r16.zxyw\n    mad r15.xyz, r16.yzxw, -r14.zxyw, r15\n    dp3 r17.x, r15, v7\n    cmp r17.x, r17.x, c44.x, c44.y\n    mul r15.xyz, r15, r17.x\n    nrm r17.xyz, v9\n    dp3 r4.x, r16, r17\n    dp3 r4.y, r15, r17\n    dp3 r4.z, r14, r17");
 }

 if(mode=="view_skin")replace(ps,"    dp3_sat r5.x, r12, r5", "    mul r14.xyz, v6, r12.x\n    mad r14.xyz, v7, r12.y, r14\n    mad r14.xyz, v8, r12.z, r14\n    nrm r15.xyz, r14\n    mov r16.x, c10.y\n    mov r16.y, c11.y\n    mov r16.z, c12.y\n    mov r17.x, -c10.x\n    mov r17.y, -c11.x\n    mov r17.z, -c12.x\n    lrp r14.xyz, c21.x, r17, r16\n    nrm r16.xyz, r14\n    dp3_sat r5.x, r15, r16");
 if(mode=="third_row"){
 ...
 if(mode=="diffuse")replace(ps,"    add_pp oC0.xyz, r0, r1","    texld r0, v0, s2\n    mov oC0.xyz, r0");
 if(mode=="flat")replace(ps,"    add_pp oC0.xyz, r0, r1","    mov oC0.xyz, c44.w");
 if(mode=="normalflat"){
  replace(ps,"    texld r9, v0, s4","    mov r9.xyz, c44.wwxw");
  replace(ps,"    texld r13, r1.zwzw, s7","    mov r13.xyz, c44.wwxw");
 }
 auto vbcode=assemble(vs),pbcode=assemble(ps);if(mode=="world_skin"||mode=="view_skin"){std::ofstream a("fixed_vs.bin",std::ios::binary);a.write((char*)vbcode->GetBufferPointer(),vbcode->GetBufferSize());std::ofstream b("fixed_ps.bin",std::ios::binary);b.write((char*)pbcode->GetBufferPointer(),pbcode->GetBufferSize());}IDirect3DVertexShader9* vertex=nullptr;IDirect3DPixelShader9* pixel=nullptr;d->CreateVertexShader((DWORD*)vbcode->GetBufferPointer(),&vertex);d->CreatePixelShader((DWORD*)pbcode->GetBufferPointer(),&pixel);vertex->Release();pixel->Release();require(SUCCEEDED(d->CreateVertexShader(skinOriginalVS,&vertex)),"original VS");require(SUCCEEDED(d->CreatePixelShader(skinOriginalPS,&pixel)),"original PS");d->SetVertexShader(vertex);d->SetPixelShader(pixel);
 bytes=read(folder+"draw_000_vs_constants.bin");d->SetVertexShaderConstantF(0,(float*)bytes.data(),256);bytes=read(folder+"draw_000_ps_constants.bin");float* c=(float*)bytes.data();
 ...
 if(mode=="ambient_only")for(int j=0;j<3;j++)c[26*4+j]=0;
 if(mode=="direct_only"){for(int j=3*4;j<10*4;j++)c[j]=0;for(int j=0;j<3;j++)c[28*4+j]=0;}
 if(mode=="fresnel_off"){c[22*4+2]=0;c[22*4+3]=0;}
 if(mode=="specular_off")for(int j=0;j<3;j++)c[13*4+j]=0;
 if(mode=="env_off")c[17*4]=c[17*4+1]=c[17*4+2]=0;d->SetPixelShaderConstantF(0,c,224);
 std::ifstream state(folder+"draw_000.txt");std::string line;std::vector<IDirect3DBaseTexture9*> textures;
 while(std::getline(state,line)){
  ...
  if(sscanf_s(line.c_str(),"texture %u %63s type=%u",&a,pointer,(unsigned)sizeof(pointer),&type)==3){
   ...
   else if(type==5){IDirect3DCubeTexture9* t=nullptr;std::string path=folder+"texture_"+pointer+".dds";hr=D3DXCreateCubeTextureFromFileA(d,path.c_str(),&t);if(FAILED(hr))return 13;tex=t;}
 ...
 IDirect3DSurface9* hdr=nullptr;d->CreateRenderTarget(1920,1080,D3DFMT_A16B16G16R16F,D3DMULTISAMPLE_NONE,0,FALSE,&hdr,nullptr);d->SetRenderTarget(0,hdr);
 ...
 d->Clear(0,nullptr,D3DCLEAR_TARGET|D3DCLEAR_ZBUFFER|D3DCLEAR_STENCIL,0xff303840,1,0);if(mode!="isolated")D3DXLoadSurfaceFromFileA(hdr,nullptr,nullptr,(folder+"draw_000_before.dds").c_str(),nullptr,D3DX_FILTER_NONE,0,nullptr);d->BeginScene(); lightingDirectionsEnabled=false;checkedDraw(d,vertex,pixel,false);
 lightingDirectionsEnabled=true;checkedDraw(d,vertex,pixel,true);
 require(lightingPairs.size()==1&&lightingPairs[0].matched,"exact captured bytecode match");
 ...
 ReleaseLightingDirections();require(!lightingDirectionsVS&&!lightingDirectionsPS&&lightingPairs.empty(),"release reset resources");
 // Recreate after release; clear so the exported image is only this draw.
 d->Clear(0,nullptr,D3DCLEAR_TARGET|D3DCLEAR_ZBUFFER,0xff303840,1,0);D3DXLoadSurfaceFromFileA(hdr,nullptr,nullptr,(folder+"draw_000_before.dds").c_str(),nullptr,D3DX_FILTER_NONE,0,nullptr);
...
d->EndScene();
 IDirect3DSurface9* back=hdr;std::string hdrOut=mode+".dds";D3DXSaveSurfaceToFileA(hdrOut.c_str(),D3DXIFF_DDS,back,nullptr,nullptr);std::string output=mode+".png";HRESULT saved=D3DXSaveSurfaceToFileA(output.c_str(),D3DXIFF_PNG,back,nullptr,nullptr);printf("mode=%s draw=%08X saved=%08X\n",mode.c_str(),hr,saved);
 ReleaseLightingDirections();return FAILED(hr)||FAILED(saved);
}
