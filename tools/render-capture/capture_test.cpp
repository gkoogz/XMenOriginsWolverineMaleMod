...
static float sliderUI[7]={50,50,50,50,50,50,50};
static void Log(const char*fmt,...){va_list a;va_start(a,fmt);vprintf(fmt,a);puts("");va_end(a);}
#include "../../src/runtime/render_capture.h"
static void require(bool ok,const char* label){if(!ok){printf("FAIL %s\n",label);fflush(stdout);ExitProcess(33);}}
...
int main(int argc,char**argv){
 ...
 strcpy_s(captureDirectory,"test_capture");CreateDirectoryA(captureDirectory,nullptr);captureRemaining=1;
 auto captureVB=read(folder+"draw_023_stream0.bin"),captureIB=read(folder+"draw_023_indices.bin");
 IDirect3DStateBlock9* beforeState=nullptr;d->CreateStateBlock(D3DSBT_ALL,&beforeState);
 float beforePC[896],afterPC[896];d->GetPixelShaderConstantF(0,beforePC,224);
 CaptureDraw(d,D3DPT_TRIANGLELIST,0,47050,3865,249804,4596,captureVB.data(),(UINT)captureVB.size(),captureIB.data(),(UINT)captureIB.size());
 CaptureSurface(d,"before");
 hr=d->DrawIndexedPrimitive(D3DPT_TRIANGLELIST,0,47050,3865,249804,4596);
 CaptureSurface(d,"after");CaptureFinish(d);
 d->GetPixelShaderConstantF(0,afterPC,224);require(!memcmp(beforePC,afterPC,sizeof(beforePC)),"constants unchanged");
 IDirect3DVertexShader9* actualVS=nullptr;IDirect3DPixelShader9* actualPS=nullptr;d->GetVertexShader(&actualVS);d->GetPixelShader(&actualPS);
 require(actualVS==vertex&&actualPS==pixel,"shaders unchanged");actualVS->Release();actualPS->Release();
 for(UINT i=0;i<16;i++){IDirect3DBaseTexture9* t=nullptr;d->GetTexture(i,&t);if(!t)continue;
 if(t->GetType()==D3DRTYPE_CUBETEXTURE){char name[MAX_PATH];sprintf_s(name,"test_capture/texture_%p.dds",t);IDirect3DCubeTexture9* cube=nullptr;
 require(SUCCEEDED(D3DXCreateCubeTextureFromFileA(d,name,&cube)),"saved cube reload");
 for(int f=0;f<6;f++){D3DLOCKED_RECT rect{};require(SUCCEEDED(cube->LockRect((D3DCUBEMAP_FACES)f,0,&rect,nullptr,D3DLOCK_READONLY)),"cube face readable");require(*(DWORD*)rect.pBits==0xff808080,"cube face exact pixels");cube->UnlockRect((D3DCUBEMAP_FACES)f,0);}cube->Release();}t->Release();}
 require(captureRemaining==0&&captureComplete&&captureDraw==1,"bounded capture completes");
 printf("PASS original shaders/constants, CPU mirrors, cubemap six-face roundtrip, capture completion\n");
 beforeState->Release();
d->EndScene();
