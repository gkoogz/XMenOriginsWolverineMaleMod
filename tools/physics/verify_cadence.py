"""Compare full-rate and alternate-frame surfaces; physics must match every frame.
...
parser.add_argument('--reference', default='2c1deee')
...
for case in [args.case] if args.case is not None else range(len(cases)):
    ...
    differences=[]
    for i,(a,b) in enumerate(zip(rows['reference'],rows['candidate'])):
        fields=[k for k in ['physics'] if a[k]!=b[k]]
        refreshed=i==0 or b['mesh']!=rows['candidate'][i-1]['mesh']
        if refreshed:fields += [k for k in ['mesh','body_positions'] if a[k]!=b[k]]
        elif b['body']!=rows['candidate'][i-1]['body']:fields.append('body changed while surface held')
        if fields:differences.append(dict(frame=int(a['frame']),fields=fields))
    fresh=[b for i,b in enumerate(rows['candidate']) if i==0 or b['mesh']!=rows['candidate'][i-1]['mesh']]
    matches=sum(a['mesh']==b['mesh'] for a,b in zip(rows['reference'],rows['candidate']))
    print('exact current-frame meshes',matches,'refreshed',len(fresh),flush=True)
    timings={variant:statistics.mean(float(r['ms']) for r in data[30:]) for variant,data in rows.items()}
    report[cases[case]]=dict(frames=len(rows['reference']),exact_current_frame_meshes=matches,refreshed_frames=len(fresh),mismatching_frames=differences,mean_cpu_ms=timings,reduction_percent=100*(1-timings['candidate']/timings['reference']),logs=logs)
