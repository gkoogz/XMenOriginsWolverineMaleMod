...
$manifest=Get-Content -LiteralPath (Join-Path $PSScriptRoot 'junction-upgrade.json') -Raw | ConvertFrom-Json
...
$backupDir=Join-Path $GamePath 'WGame\ModBackups\WolverineAnatomyTool-v0.7.1-junction-r2'
...
$current=Hash $target
if($current -eq $manifest.runtimeSHA256){Write-Output 'v0.7.1 junction R2 is already installed.';return}
if($manifest.acceptedRuntimeSHA256 -notcontains $current){throw 'This upgrade requires the released 0.7 or original 0.7.1 runtime. No files were changed.'}
...
$state=@{Version='0.7.1-junction-r2';GamePath=$GamePath;Status='Pending';OriginalReadOnly=$readOnly;PreviousHash=$current;InstalledHash=$manifest.runtimeSHA256}
...
Write-Output 'v0.7.1 junction R2 installed. Launch normally; F6 shows the version. Package and saved settings were preserved.'
